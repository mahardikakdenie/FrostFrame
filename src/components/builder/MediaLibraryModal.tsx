    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
        ...
        <motion.div 
          ...
          className="relative bg-white w-full max-w-5xl h-[80vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-white/20"
        >
          {/* Header */}
          <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between shrink-0">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <h3 className="text-lg font-black uppercase italic tracking-tighter text-slate-900 leading-none">Media Library</h3>
                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Lando Studio Assets</span>
                </div>
             </div>
            ...
            <div className="px-8 py-4 bg-slate-50 border-b border-slate-100 shrink-0">
               <form onSubmit={handleSearch} className="relative max-w-md">
                  ...
                  <input 
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm"
                    value={searchQuery}
             ...
             {activeTab === 'unsplash' ? (
                isLoading ? (
                  <div className="h-full flex flex-col items-center justify-center gap-4 opacity-50">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest italic">Fetching Masterpieces...</span>
                  </div>
                ) : images.length > 0 ? (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                     {images.map((img) => (
                        <motion.div 
                          ...
                          className="group relative aspect-square rounded-2xl overflow-hidden cursor-pointer shadow-lg bg-slate-100"
                        >
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-30 italic">
                     ...
                     <p className="text-sm font-black">No images found. Try a different keyword.</p>
                  </div>
                )
             ) : (
